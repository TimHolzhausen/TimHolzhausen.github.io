...
// Connection
// ============================================================
async function connectPrinter(useAllDevices = false) {
  ...
  hideAllDevicesBtn();
  try {
    await state.printer.connect(useAllDevices);
    ...
    const userCancelled = e.name === 'NotFoundError' || e.message.includes('User cancelled');
    if (!useAllDevices && !userCancelled) {
      // Service-UUID filter returned nothing → offer "show all" fallback
      showAllDevicesBtn();
      showToast('⚠️ Drucker nicht gefunden. Versuche "Alle Geräte anzeigen".', 'warning', 6000);
    } else if (!userCancelled) {
      showToast(`❌ Fehler: ${e.message}`, 'error', 7000);
    }
    btn.querySelector('span').textContent = 'Verbinden';
    updateConnectionStatus('error', userCancelled ? 'Nicht verbunden' : 'Verbindungsfehler');
  } finally {
    btn.disabled = false;
  }
}

function showAllDevicesBtn() {
  let btn = document.getElementById('btn-connect-all');
  if (!btn) {
    btn = document.createElement('button');
    btn.id = 'btn-connect-all';
    btn.className = 'btn btn-secondary';
    btn.style.marginTop = '8px';
    btn.innerHTML = `<svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg><span>Alle Geräte anzeigen (Android-Fix)</span>`;
    btn.onclick = () => connectPrinter(true);
    const area = document.querySelector('.connection-area');
    if (area) {
