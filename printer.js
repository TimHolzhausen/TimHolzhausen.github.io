...
class PeripagePrinter {
  ...
  // useAllDevices: if true, show all BLE devices (useful on Android)
  // -------------------------------------------------------
  async connect(useAllDevices = false) {
    if (!navigator.bluetooth) {
      throw new Error('Web Bluetooth wird von diesem Browser nicht unterstützt. Bitte Chrome oder Edge (mit aktiviertem Bluetooth) verwenden.');
    }

    this._log('info', 'Suche nach Peripage P21...');
    this._status('connecting', 'Verbinde...');

    // Strategy A: filter by known Peripage BLE service UUID
    // → Only Peripage/compatible printers appear in the list (cleaner UX)
    // Strategy B: acceptAllDevices → all BLE devices visible (Android fix)
    //
    // On Android Chrome, namePrefix-filters often show an empty dialog if
    // the device name reported via BLE advertisement doesn't exactly match.
    // The services-filter is more reliable: it matches on GATT service UUID.
    // If that still fails (some firmwares don't advertise UUID), fall back to all.

    if (!useAllDevices) {
      try {
        // Try service UUID filter first — most reliable cross-platform
        this.device = await navigator.bluetooth.requestDevice({
          filters: [
            { services: [PeripagePrinter.SERVICE_UUID] },
          ],
          optionalServices: [PeripagePrinter.SERVICE_UUID],
        });
        this._log('info', 'Service-Filter erfolgreich');
      } catch (filterErr) {
        this._log('warn', `Service-Filter fehlgeschlagen (${filterErr.message}), zeige alle Geräte...`);
        // This second call requires the same user gesture (button click) to still be active.
        // On most browsers it works if the first call was rejected programmatically (not by user).
        if (filterErr.name === 'NotFoundError' || filterErr.name === 'NotSupportedError') {
          // Re-throw so caller can offer the "show all" button
          throw filterErr;
        }
      ...
      // Show ALL BLE devices — user must manually select the printer
      this._log('info', 'Zeige alle BLE-Geräte (acceptAllDevices)...');
      this.device = await navigator.bluetooth.requestDevice({
